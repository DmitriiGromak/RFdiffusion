...
class PotentialManager:
    ...
    def __init__(self,
        ...
        self.potentials_to_apply, self.potentials_to_apply_seq = self.initialize_all_potentials(setting_list)
    ...
    def initialize_all_potentials(self, setting_list):
        ...
        for potential_dict in setting_list:
            assert (potential_dict[
                        'type'] in potentials.implemented_potentials), f'potential with name: {potential_dict["type"]} is not one of the implemented potentials: {potentials.implemented_potentials.keys()}'
            ...
            if potential_dict['type'] in potentials.require_seq:
                to_apply_seq.append(potentials.implemented_potentials[potential_dict['type']](**kwargs))
            else:
                to_apply.append(potentials.implemented_potentials[potential_dict['type']](**kwargs))

        return to_apply, to_apply_seq
    ...
    def compute_all_potentials_seq(self, xyz, seq):
        '''
            Same as compute_all_potentials, but for potential, using sequence
        '''

        potential_list = [potential.compute(xyz, seq) for potential in self.potentials_to_apply_seq]
        potential_stack = torch.stack(potential_list, dim=0)
    ...
    def get_guide_scale_seq(self, t):
        '''
        Given a timestep, get the appropriate scale factor to use for applying guiding potentials with sequence
